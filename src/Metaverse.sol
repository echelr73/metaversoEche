...
//Creation of the Metarverse Smart Contract with NFT Tokens
contract Metaverse is ERC721, Ownable{
    ...
    // List od Metaverse buildings
    Building[] public buildings;

    // Obtaining the buildgs made in the Metaverse
    ...
    function mint(string memory _building_name, int8 _w, int8 _h, int8 _d, int8 _x, int8 _y, int8 _z, string memory _building_material) public payable {
        require (supply.current() <= maxSupply, "Max supply exceeded!");
        require (msg.value >= cost, "Insufficient funds!");
        supply.increment();
        _safeMint(msg.sender, supply.current());
        Building memory _newBuild = Building(_building_name, _w, _h, _d, _x, _y, _z, _building_material);
